...
def plot_loss_comparison_per_rank(
    ...
    for process_id in sorted(all_process_ids):
        ...
        losses1 = [loss for _, loss in process_losses1.get(process_id, [])]
        losses2 = [loss for _, loss in process_losses2.get(process_id, [])]

        steps1 = list(range(len(losses1)))
        steps2 = list(range(len(losses2)))
        ...
        print(
            f"Process {process_id} - aligned steps: {min_len} "
            f"({label1} {len(process_losses1.get(process_id, []))}, "
            f"{label2} {len(process_losses2.get(process_id, []))})"
        )
...
def plot_relative_diff_per_rank(
    ...
    for process_id in sorted(common_process_ids):
        ...
        losses1_arr = np.array(losses1[:min_len])
        losses2_arr = np.array(losses2[:min_len])
